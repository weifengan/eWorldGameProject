...
namespace Xft
{
    ...
    public class XftGlowPerObj : MonoBehaviour
    {
        ...
        void PrepareRenderTex()
        {
            if (TempRenderTex == null)
            {
                TempRenderTex = RenderTexture.GetTemporary((int)GetComponent<Camera>().pixelWidth, (int)GetComponent<Camera>().pixelHeight, 0);
            }

            if (TempRenderGlow == null)
            {
                TempRenderGlow = RenderTexture.GetTemporary((int)GetComponent<Camera>().pixelWidth, (int)GetComponent<Camera>().pixelHeight, 0);
            }
